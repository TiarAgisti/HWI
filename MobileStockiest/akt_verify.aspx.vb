...
Partial Class MobileStockiest_akt_verify
    ...
    Sub PrepareData()
        ...
        nktp = mlFUNCT.SafeSQL(UCase(Request("ktp")))
        kelamin = Request("kelamin")
        tgllahir = Request("tgllahir")
        bulanlahir = Request("bulanlahir")
        tahunlahir = Request("tahunlahir")
        alamat = mlFUNCT.SafeSQL(UCase(Request("alamat")))
        kota = mlFUNCT.SafeSQL(UCase(Request("kota")))
        propinsi = Request("propinsi")
        kodepos = mlFUNCT.SafeSQL(UCase(Request("kodepos")))

        surat = mlFUNCT.SafeSQL(UCase(Request("surat")))
        kotasurat = mlFUNCT.SafeSQL(UCase(Request("kotasurat")))
        propinsisurat = Request("propinsisurat")
        kodepossurat = mlFUNCT.SafeSQL(UCase(Request("kodepossurat")))

        notelp = mlFUNCT.SafeSQL(UCase(Request("notelp")))
        nohape = mlFUNCT.SafeSQL(UCase(Request("nohape")))
        pasangan = Request("pasangan")
        ahliwaris = mlFUNCT.SafeSQL(UCase(Request("ahliwaris")))
        hubwaris = Request("hubwaris")
        eml = mlFUNCT.SafeSQL(UCase(Request("email")))

        bnk = mlFUNCT.SafeSQL(UCase(Request("namabank")))
        nrk = mlFUNCT.SafeSQL(UCase(Request("norek")))
        namarek = mlFUNCT.SafeSQL(UCase(Request("namarek")))

        If ncrd = "" Then
            l1 = "Mbuh"
            error1 = "Nomor seri pendaftaran / nomor distributor belum anda isikan"
        Else
            ...
            '		if len(ncrd) > 7 then
            '			error1 = "Nomor seri pendaftaran / nomor distributor maksimal 7 karakter"
            '			l1 = "Mbuh"		
            '		else
            '		if len(ncrd) < 7 then
            '			error1 = "Nomor seri pendaftaran / nomor distributor minimal 7 karakter"
            '			l1 = "Mbuh"		
            '		else			
            ...
            '				else
            '					nama = rs("uid")
            '					error1 = "Nomor seri pendaftaran / nomor distributor anda isikan sudah digunakan oleh distributor lain, yaitu : "&nama
            '					l1 = "Mbuh"
            '				end if
            '			rs.close 
            '		end if
